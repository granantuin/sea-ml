...
try:
  ...
  for t in range(0,24):
    ...
    color = 'tab:red'
    ax1.set_xlabel('Time:{} Z'.format(t+1))
    ax1.set_ylabel('wind_speed kt', color=color)
    ax1.plot(df_h.index, df_h['spd'], color=color)
    ...
    ax2 = ax1.twinx()
    color = 'tab:blue'
    ax2.set_ylabel('wind_direction degrees', color=color)
    ax2.plot(df_h.index, df_h['dir'], color=color)
    ax2.tick_params(axis='y', labelcolor=color)
    ax2.grid(True)
 
    # Title
    plt.title('wind speed (kt) versus wind direction (degrees). Forecast day 0')
...
try:
  ...
  df_mod=pd.DataFrame({"time":meteo_model[:72].index,
                       ...
                       "WRF_prec0": meteo_model.prec0[:72]})
